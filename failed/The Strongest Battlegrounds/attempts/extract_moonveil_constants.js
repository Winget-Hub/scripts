...
const inputPath = path.join(artifactRoot, "TsbMainScript.lua");
...
const helperMatch = source.match(/([A-Za-z_][A-Za-z0-9_]*)=function\([A-Za-z_][A-Za-z0-9_]*,[A-Za-z_][A-Za-z0-9_]*\)/);
const helperName = helperMatch ? helperMatch[1] : "yd";
const stringLiteral = String.raw`(?:"((?:\\.|[^"\\])*)"|'((?:\\.|[^'\\])*)')`;
const callPattern = new RegExp(String.raw`${helperName}\(\s*${stringLiteral}\s*,\s*${stringLiteral}\s*\)`, "g");
...
fs.writeFileSync(
  ...
  [
    `Input: ${inputPath}`,
    `Detected XOR helper: ${helperName}`,
    `${helperName} calls decoded: ${decoded.length}`,
